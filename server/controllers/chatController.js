...
export const getUserChats = async (req, res) => {
  try {
    ...
    const chats = await Chat.findAll({
      where: {
        [Op.or]: [{ customerId: userId }, { ownerId: userId }],
      },
    ...
    const chatsWithDetails = await Promise.all(
      chats.map(async (chat) => {
        ...
        if (chat.customerId === userId) {
          ...
          if (otherUser) {
            otherUserName =
              `${otherUser.firstName} ${otherUser.middleName ? otherUser.middleName + " " : ""}${otherUser.lastName}`.trim();
            otherUserImage =
              otherUser.profileImage || otherUser.selfie || otherUser.idPhoto;
          }
        } else {
