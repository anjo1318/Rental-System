...
const fetchItemById = async (req, res) => {
  console.log("Fetching item by ID in fetchItemById:", req.params.id);
  try {
    const { id } = req.params;
    ...
    // Fetch reviews using itemId directly, no association needed
      ...
      include: [
        {
          ...
          attributes: [
            ...
            "idPhoto",
            "gcashQR",
          ], // Changed from "email" to "emailAddress"
        },
      ],
      order: [["createdAt", "DESC"]],
    });
  ...
  try {
    ...
    if (itemImages) {
      if (typeof itemImages === "string") {
        ...
        processedImages = itemImages.filter(
